...
AnalysisResult SemanticAnalyzer::analyze(const parser::Program& program) {
...

    {
        ...
        if (it == functions_.end()) {
            ...
            for (auto& info : overloads) {
                if (info.param_types.empty()) {
                    if (info.return_type != "int32" && info.return_type != "int64")
                        diagnostics_.push_back(Diagnostic{filename_, 0, 0, "функция 'main' должна возвращать int32 или int64"});
                    found_valid = true;
                    break;
                }
            }
...
// ищем подходящую перегрузку метода
    std::vector<std::string> arg_types;
    for (auto& a : node.args)
        arg_types.push_back(checkExpr(*a));
    const FuncInfo* matched_method = nullptr;
    for (auto& overload : fit->second) {
        if (overload.param_types.size() != arg_types.size() + 1) continue;
        bool ok = true;
        for (size_t i = 0; i < arg_types.size(); ++i)
            if (!typesCompatible(overload.param_types[i + 1], arg_types[i])) { ok = false; break; }
        if (ok) { matched_method = &overload; break; }
    }
    if (!matched_method) {
        error(node.pos.line, node.pos.col, "нет подходящей перегрузки метода '" + fa->field + "'");
        return "";
    }
    return matched_method->return_type;
}
